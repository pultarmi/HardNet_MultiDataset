...
from utils_ import batch_eig2x2, batched_forward
from Learning.LAF import angles2A, get_inverted_pyr_index, denormalizeLAFs, extract_patches_from_pyramid_with_inv_index
from Learning.LAF import get_pyramid_and_level_index_for_LAFs, normalizeLAFs, checkTouchBoundary
from Learning.handcrafted_modules import HessianResp, AffineShapeEstimator, OrientationDetector, ScalePyramid, NMS3dAndComposeA
...
class ScaleSpaceAffinePatchExtractor(nn.Module):
    def __init__(self, 
        ...
        self.mrSize = mrSize
        self.PS = patch_size ### PATCH SIZE IS NOT ACTIVE??????
        self.b = border
    ...
    def multiScaleDetector(self, x, num_features = 0):
        ...
        for oct_idx in range(len(self.sigmas)):
            ...
            for level_idx in range(1, len(octave)-1):
                ...
                pyr_id = oct_idx * torch.ones(aff_matrix.size(0))
                # lev_id = Variable((level_idx - 1) * torch.ones(aff_matrix.size(0))) #prevBlur
                lev_id = (level_idx - 1) * torch.ones(aff_matrix.size(0))
                if x.is_cuda:
                    pyr_id = pyr_id.cuda()
                    lev_id = lev_id.cuda()
