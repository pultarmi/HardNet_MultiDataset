...
def get_WF_from_string(weight_function, sum_to_one=False):
    HR = HessianResp()

    def normit(tensor):
        return tensor / np.sum(tensor)

    def HRplain(pilimg, sum_to_one=False):
        with torch.no_grad():
            if type(pilimg) == torch.Tensor:
                res = HR.cuda()(pilimg.unsqueeze(0).mean(dim=1, keepdim = True).cuda(), 1.0).squeeze().cpu().data.numpy()
            else:
                res = HR.cuda()((transforms.ToTensor()(pilimg)).unsqueeze(0).mean(dim=1, keepdim = True).cuda(), 1.0).squeeze().cpu().data.numpy()
            return normit(res) if sum_to_one else res
    def HRSqrt(pilimg):
        with torch.no_grad():
            if type(pilimg) == torch.Tensor:
                res = np.sqrt(HR.cuda()(pilimg.unsqueeze(0).mean(dim=1, keepdim = True).cuda(), 1.0).squeeze().cpu().data.numpy())
            else:
                res = np.sqrt(HR.cuda()((transforms.ToTensor()(pilimg)).unsqueeze(0).mean(dim=1, keepdim = True).cuda(), 1.0).squeeze().cpu().data.numpy())
            return normit(res) if sum_to_one else res
    def HRSqrt4(pilimg, sum_to_one=False):
        with torch.no_grad():
            if type(pilimg) == torch.Tensor:
                res = np.sqrt(np.sqrt(HR.cuda()(pilimg.unsqueeze(0).mean(dim=1, keepdim = True).cuda(), 1.0).squeeze().cpu().data.numpy()))
            else:
                res = np.sqrt(np.sqrt(HR.cuda()((transforms.ToTensor()(pilimg)).unsqueeze(0).mean(dim=1, keepdim = True).cuda(), 1.0).squeeze().cpu().data.numpy()))
            return normit(res) if sum_to_one else res
    ...
    if weight_function == 'Hessian':
        ...
        WF = HRSqrt4
    elif weight_function == 'None':
        WF = None
    else: print('Unknown weight function', weight_function, 'using uniform weight sampling')
