...
#HardNet margin loss - calculates loss based on distance matrix based on positive distance and closest negative distance.
    ...
    pos = distance_vectors_pairwise(anchor,positive)
    dist_matrix_p_detach = distance_matrix_vector(anchor, positive.detach())

    eye = torch.autograd.Variable(torch.eye(dist_matrix_p_detach.size(1))).cuda()
    dist_without_min_on_diag = dist_matrix_p_detach + eye*10
    mask = (dist_without_min_on_diag.ge(0.008).float()-1.0)*(-1)
    mask = mask.type_as(dist_without_min_on_diag)*10
    dist_without_min_on_diag = dist_without_min_on_diag + mask
    min_neg = torch.min(dist_without_min_on_diag,1)[0]
    if False:
        dist_matrix_a_detach = distance_matrix_vector(anchor.detach(), positive)
        dist_without_min_on_diag2 = dist_matrix_a_detach + eye*10
        mask2 = (dist_without_min_on_diag2.ge(0.008).float()-1.0)*(-1)
...
def loss_HardNetCore( anchor, positive, anchor_swap = False, anchor_ave = False, margin = 1.0,pos_margin = 0.05,  batch_reduce = 'min', loss_type = "triplet_margin" ):
# HardNet margin loss - calculates loss based on distance matrix based on positive distance and closest negative distance.
    assert anchor.size() == positive.size(), "Input sizes between positive and negative must be equal."
    assert anchor.dim() == 2, "Inputd must be a 2D matrix."
    eps = 1e-8
    ...
    if loss_type == "triplet_margin":
        loss = torch.clamp(margin  + torch.clamp(pos - pos_margin, min = pos_margin, max = 2.0) - min_neg, min=0.0)
    elif loss_type == 'softmax':
        exp_pos = torch.exp(2.0 - pos);
        exp_den = exp_pos + torch.exp(2.0 - min_neg) + eps;
        loss = - torch.log( exp_pos / exp_den )
...
def loss_HardNetSqrt( anchor, positive, anchor_swap = False, anchor_ave = False, margin = 1.0, batch_reduce = 'min', loss_type = "triplet_margin" ):
# HardNet margin loss - calculates loss based on distance matrix based on positive distance and closest negative distance.
    assert anchor.size() == positive.size(), "Input sizes between positive and negative must be equal."
    assert anchor.dim() == 2, "Inputd must be a 2D matrix."
    eps = 1e-8
    ...
    if loss_type == "triplet_margin":
        ...
        print ('Unknown loss type. Try triplet_margin, softmax or contrastive')
        sys.exit(1)
    loss = torch.mean(torch.sqrt(torch.sqrt(loss+ 1e-10)))
    return loss

def loss_HardNetSq( anchor, positive, anchor_swap = False, anchor_ave = False, margin = 1.0, batch_reduce = 'min', loss_type = "triplet_margin" ):
# HardNet margin loss - calculates loss based on distance matrix based on positive distance and closest negative distance.
    assert anchor.size() == positive.size(), "Input sizes between positive and negative must be equal."
    assert anchor.dim() == 2, "Inputd must be a 2D matrix."
    eps = 1e-8
