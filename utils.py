...
def get_last_checkpoint(dir):
    if not os.path.exists(os.path.join(dir, 'checkpoint_0.pth')):
        return None
    else:
        last = 'checkpoint_0.pth'
    i = 1
    while True:
        text = 'checkpoint_{}.pth'.format(i)
        if not os.path.exists(os.path.join(dir, text)):
            return last
        last = text
        i += 1

def send_email(recipient, ignore_host=''):
    msg = MIMEText('')
